...
describe('Transaction', function() {
  ...
  it("constructs Transaction object from a TransactionEnvelope", function(done) {
    let source      = new RoverBase.Account("GBBM6BKZPEHWYO3E3YKREDPQXMS4VK35YLNU7NFBRI26RAN7GI5POFBB", "0");
    let destination = "GDJJRRMBK4IWLEPJGIE6SXD2LP7REGZODU7WDC3I2D6MR37F4XSHBKX2";
    let asset       = RoverBase.Asset.native();
    let amount      = "2000";

    let input = new RoverBase.TransactionBuilder(source)
                .addOperation(RoverBase.Operation.payment({destination, asset, amount}))
                .addMemo(RoverBase.Memo.text('Happy birthday!'))
                .build()
                .toEnvelope()
                .toXDR('base64');


    var transaction = new RoverBase.Transaction(input);
    ...
    expect(transaction.memo.type).to.be.equal(RoverBase.MemoText);
  ...
  it("does not sign when no Network selected", function() {
    ...
    let asset       = RoverBase.Asset.native();
    let amount      = "2000";
    let signer      = RoverBase.Keypair.random();

    let tx = new RoverBase.TransactionBuilder(source)
                .addOperation(RoverBase.Operation.payment({destination, asset, amount}))
                .build();
  ...
  it("signs correctly", function() {
    ...
    let asset       = RoverBase.Asset.native();
    let amount      = "2000";
    let signer      = RoverBase.Keypair.master();
  ...
  it("accepts 0 as a valid transaction fee", function(done) {
    ...
    let input = new RoverBase.TransactionBuilder(source, {fee: 0})
                .addOperation(RoverBase.Operation.payment({destination, asset, amount}))
                .addMemo(RoverBase.Memo.text('Happy birthday!'))
                .build()
                .toEnvelope()
