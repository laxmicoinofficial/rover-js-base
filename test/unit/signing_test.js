...
describe('RoverBase#sign', function() {
  ...
  it("can sign an string properly", function() {
    ...
    let actualSig   = RoverBase.sign(data, secretKey).toString('hex');
...
describe('RoverBase#verify', function() {
  ...
  it("can verify an string properly", function() {
    let data = "hello world";
    expect(RoverBase.verify(data, sig, publicKey)).to.be.truthy;
    expect(RoverBase.verify("corrupted", sig, publicKey)).to.be.falsey;
    expect(RoverBase.verify(data, badSig, publicKey)).to.be.falsey;
  });
