...
describe('Keypair.contructor', function() {
  it("fails when passes secret key does not match public key", function() {
    ...
    let kp = RoverBase.Keypair.fromSecret(secret);

    let secretKey = kp.rawSecretKey();
    let publicKey = RoverBase.StrKey.decodeEd25519PublicKey(kp.publicKey());
    publicKey[0] = 0; // Make public key invalid

    expect(() => new RoverBase.Keypair({type: 'ed25519', secretKey, publicKey})).to.throw(/secretKey does not match publicKey/)
  });

  it("fails when secretKey length is invalid", function() {
    let secretKey = new Buffer(33);
    expect(() => new RoverBase.Keypair({type: 'ed25519', secretKey})).to.throw(/secretKey length is invalid/)
  });

  it("fails when publicKey length is invalid", function() {
    let publicKey = new Buffer(33);
    expect(() => new RoverBase.Keypair({type: 'ed25519', publicKey})).to.throw(/publicKey length is invalid/)
  });
});
...
describe('Keypair.fromSecret', function() {
  ...
  it("creates a keypair correctly", function() {
    ...
    expect(kp).to.be.instanceof(RoverBase.Keypair);
  ...
  it("throw an error if the arg isn't strkey encoded as a seed", function() {
    ...
    expect(() => RoverBase.Keypair.fromSecret("hel0")).to.throw()
    expect(() => RoverBase.Keypair.fromSecret("SBWUBZ3SIPLLF5CCXLWUB2Z6UBTYAW34KVXOLRQ5HDAZG4ZY7MHNBWJ1")).to.throw()
    expect(() => RoverBase.Keypair.fromSecret("masterpassphrasemasterpassphrase")).to.throw()
    expect(() => RoverBase.Keypair.fromSecret("gsYRSEQhTffqA9opPepAENCr2WG6z5iBHHubxxbRzWaHf8FBWcu")).to.throw()
...
describe('Keypair.fromBase58Seed', function() {
  ...
  it("creates a keypair correctly", function() {
    ...
    for (let i in keys) {
      let key = keys[i];
      let keyPair = RoverBase.Keypair.fromBase58Seed(key.oldSeed);
  ...
  it("throw an error if the arg isn't base58 encoded as a seed", function() {
    ...
    expect(() => RoverBase.Keypair.fromBase58Seed("hel0")).to.throw()
    expect(() => RoverBase.Keypair.fromBase58Seed("masterpassphrasemasterpassphrase")).to.throw()
    expect(() => RoverBase.Keypair.fromBase58Seed("gsYRSEQhTffqA9opPepAENCr2WG6z5iBHHubxxbRzWaHf8FBWcu")).to.throw()
...
describe('Keypair.fromRawEd25519Seed', function() {
  ...
  it("creates a keypair correctly", function() {
    ...
    let kp = RoverBase.Keypair.fromRawEd25519Seed(seed);
  ...
  it("throws an error if the arg isn't 32 bytes", function() {
    expect(() => RoverBase.Keypair.fromRawEd25519Seed("masterpassphrasemasterpassphras")).to.throw()
    expect(() => RoverBase.Keypair.fromRawEd25519Seed("masterpassphrasemasterpassphrase1")).to.throw()
    expect(() => RoverBase.Keypair.fromRawEd25519Seed(null)).to.throw()
    expect(() => RoverBase.Keypair.fromRawEd25519Seed()).to.throw()
  });
...
describe('Keypair.fromPublicKey', function() {
  ...
  it("creates a keypair correctly", function() {
    let kp = RoverBase.Keypair.fromPublicKey("GAXDYNIBA5E4DXR5TJN522RRYESFQ5UNUXHIPTFGVLLD5O5K552DF5ZH");
    expect(kp).to.be.instanceof(RoverBase.Keypair);
    expect(kp.publicKey()).to.eql("GAXDYNIBA5E4DXR5TJN522RRYESFQ5UNUXHIPTFGVLLD5O5K552DF5ZH");
    expect(kp.rawPublicKey().toString("hex")).to.eql("2e3c35010749c1de3d9a5bdd6a31c12458768da5ce87cca6aad63ebbaaef7432");
  });
  ...
  it("throw an error if the arg isn't strkey encoded as a accountid", function() {
    expect(() => RoverBase.Keypair.fromPublicKey("hel0")).to.throw()
    expect(() => RoverBase.Keypair.fromPublicKey("masterpassphrasemasterpassphrase")).to.throw()
    expect(() => RoverBase.Keypair.fromPublicKey("sfyjodTxbwLtRToZvi6yQ1KnpZriwTJ7n6nrASFR6goRviCU3Ff")).to.throw()
  });

  it("throws an error if the address isn't 32 bytes", function() {
    expect(() => RoverBase.Keypair.fromPublicKey("masterpassphrasemasterpassphrase")).to.throw()
    expect(() => RoverBase.Keypair.fromPublicKey("masterpassphrasemasterpassphrase")).to.throw()
    expect(() => RoverBase.Keypair.fromPublicKey(null)).to.throw()
    expect(() => RoverBase.Keypair.fromPublicKey()).to.throw()
  });
...
describe('Keypair.random', function() {
  ...
  it("creates a keypair correctly", function() {
    let kp = RoverBase.Keypair.random();
    expect(kp).to.be.instanceof(RoverBase.Keypair);
  });
