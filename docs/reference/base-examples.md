...
The source account is giving the new account 25 XLX as its initial balance. Current sequence number of the source account in the ledger is `46316927324160`.
...
var an_account = new RoverSdk.Account("GCEZWKCA5VLDNRLN3RPRJMRZOX3Z6G5CHCGSNFHEYVXM3XOJMDS674JZ", 46316927324160);

var transaction = new RoverSdk.TransactionBuilder(an_account)
    .addOperation(RoverSdk.Operation.createAccount({
      destination: "GASOCNHNNLYFNMDJYQ3XFMI7BYHIOCFW3GJEOWRPEGK2TDPGTG2E5EDW",
      startingBalance: "25"  // in XLX
    }))
    .build();

transaction.sign(RoverSdk.Keypair.fromSecret(seedString)); // sign the transaction
...
## Assets
Object of the `Asset` class represents an asset in the Rover network. Right now there are 3 possible types of assets in the Rover network:
* native `XLX` asset (`ASSET_TYPE_NATIVE`),
...
In the example below we're sending 1000 XLX (at max) from `GABJLI6IVBKJ7HIC5NN7HHDCIEW3CMWQ2DWYHREQQUFWSWZ2CDAMZZX4` to
...
The [path payment](http://www.rover.network/developers/guides/concepts/list-of-operations.html#path-payment) will cause the destination address to get 5.5 GBP. It will cost the sender no more than 1000 XLX. In this example there will be 3 exchanges, XLX -> USD, USD-> EUR, EUR->GBP.

```js
RoverSdk.Network.useTestNetwork();
var keypair=RoverSdk.Keypair.fromSecret(secretString);
...
[Multi-signature accounts](http://www.rover.network/developers/guides/concepts/multi-sig.html) can be used to require that transactions require multiple public keys to sign before they are considered valid.
